...
export default class SvgColor extends Component {
    ...
    async getSvgData() {
		...
		if (this.state.data.svg !== data)
			this.setState({ svg: data })
    }

    render() {
        let { svg } = this.state
        let { colors, width, rotate, style, className, onClick } = this.props
        ...
        if(svg != null && isSvg(svg)) {
            ...
            return (
                React.createElement("div", null,
                    React.createElement("img", {src: svg, style: style, className: className, onClick: onClick})
                )
            )
        }
        else {
